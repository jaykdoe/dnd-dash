...
const MetricTableChart = ({ metric }: { metric: TYPES.Metric }) => {
  ...
  return (
    <div className="overflow-x-auto relative rounded-t-lg">
      <table className="w-full">
        <thead>
          <tr
            className={`dark:bg-slate-700 bg-${color.hardClass} dark:text-${color.hardClass} text-white text-sm`}
          >
            <th className="font-light text-left px-3 py-2">
              {metric.header.dimension}
            </th>
            <th className="font-light text-right px-3 py-2">
              {metric.header.value}
            </th>
          </tr>
        </thead>
          ...
          {metric.data.map((data, index) => (
            <tr
              ...
              className={classNames([
                index % 2 === 0
                  ? `dark:bg-slate-900 bg-${color.softClass}`
                  : "dark:bg-slate-800",
